...
static struct attribute *msi_attrs[] = {
	&msi_attribute.attr,
	NULL,	/* need to NULL terminate the list of attributes */
};

ATTRIBUTE_GROUPS(msi);
...
void penmsi_remove(struct platform_device *pdev)
{
	...
	msi_domain_free_irqs_all(&pdev->dev, 0);
	del_attrs(platdata);
...
int penmsi_probe(struct platform_device *pdev)
{
	...
	rc = msi_domain_alloc_irqs(&pdev->dev, 0, num_interrupts);
	if (rc != 0) {
		dev_err(&pdev->dev, "msi_domain_alloc_irqs failed: %d\n", rc);
		goto up_sem;
	}
